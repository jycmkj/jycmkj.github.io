...
/*
var _navWaitlistExpanded = false;
$(function() {
    registerNavWaitlist();
    registerInputBlur();
});

function registerNavWaitlist() {
    registerAjaxForm($('#nav_waitlist_form'), handleWaitlistSuccess, handleWaitlistError);
    $('#nav_waitlist_form input').placeholder();
    $('#nav_waitlist_button').click(function() {
        if (_navWaitlistExpanded) {
            return true;
        }
        expandNavWaitlistForm();
        return false;
    });
}

function expandNavWaitlistForm() {
    $('#nav_waitlist_form').removeClass('collapsed');
    $('#nav_email_input').focus();
    $('#nav_waitlist_button').html('OK');
    _navWaitlistExpanded = true;
}

function collapseNavWaitlistForm() {
    $('#nav_waitlist_form').addClass('collapsed');
    $('#nav_waitlist_button').html('下載產品目錄');
    _navWaitlistExpanded = false;
}
*/
/*
function registerInputBlur() {
    $(document).click(function(e) {
        ...
        var target = $(e.target);
        if (target.is('#nav_email_input,#nav_waitlist_button')) {
            return;
        }
        collapseNavWaitlistForm();
    });
}
*/
...
function handleWaitlistError() {
    var email_input = $(this).find('.waitlist-email');
    email_input.parsley('manageError', {
        name: 'server-error'
    });
    email_input.addClass('parsley-error');
}! function(a) {
    var b = {
            attr: function(a, b, c) {
                var d, e = {},
...
function registerNavScrollHandler() {
    var button = $('#home_waitlist_button');
    _navTriggerY = button.offset().top + button.outerHeight();
    _nav = $('#nav');
    _navHiddenTop = _nav.css('top');
    $(window).scroll($.throttle(200, showOrHideNav));
}

function showOrHideNav() {
    var window_y = $(window).scrollTop();
    if (_navHidden && window_y > _navTriggerY) {
        _nav.addClass('exposed');
        _navHidden = false;
    } else if (!_navHidden && window_y < _navTriggerY) {
        _nav.removeClass('exposed');
//        collapseNavWaitlistForm();
        _navHidden = true;
    }
}
...
var MIN_ANIM_WINDOW_WIDTH = 768;
var ANIM_SCROLL_START = 150;

function animationSupported() {
    if ($(window).width() < MIN_ANIM_WINDOW_WIDTH) {
        return false;
    }
    var elem = document.body || document.element;
    if (typeof elem.style['transition'] == 'string' || typeof elem.style['WebkitTransition'] == 'string') {
        return true;
    }
    return false;
}
$(function() {
    var placeholders = $('#splash_phones_wrapper,#pricing_feature_image');
    if (!animationSupported()) {
        placeholders.addClass('visible');
        return;
    }
    placeholders.remove();
    initWatchlist();
...
var _animationHandler = $.throttle(200, beginWatchlistAnimation);

function registerAnimationHandler() {
    $(window).scroll(_animationHandler);
    beginWatchlistAnimation();
}

function beginWatchlistAnimation() {
    var window_y = $(window).scrollTop();
    if (window_y >= ANIM_SCROLL_START) {
        $(window).off('scroll', _animationHandler);
        tapROBN();
    }
}
var _watchlistRoot;
var _watchlistPress;
var _watchlistAnimationRoot;
var _watchlistSlideBuy;
var _dayNightSrcSwap;
var _transformMask;
var _isDaytime = true;

function initWatchlist() {
    _dayNightSrcSwap = $('.day-night-src-swap');
    _transformMask = $('.absolute-element');
    _transformMask.show();
    _watchlistRoot = $('#watchlist_root');
    $('.watchlist-version').removeClass('hidden');
}

function tapROBN() {
    if (_isDaytime) {
        _watchlistAnimationRoot = $('#white_wrapper');
    } else {
        _watchlistAnimationRoot = $('#black_wrapper');
    }
    _watchlistAnimationRoot.removeClass('to-portfolio');
    _watchlistPress = _watchlistAnimationRoot.find('.watchlist-press');
    _watchlistPress.removeClass('med-tap');
    $('.transition-mask').removeClass('fade-in');
    _watchlistPress.addClass('tap');
    setTimeout(showROBN, 750);
}

function showROBN() {
    _watchlistPress.removeClass('tap');
    _watchlistSlideBuy = _watchlistAnimationRoot.find('.watchlist-slide-buy');
    _watchlistSlideBuy.hide();
    _watchlistAnimationRoot.addClass('detail-view');
    setTimeout(holdDetail, 2000);
}

function holdDetail() {
    _watchlistPress.addClass('long-tap');
    setTimeout(showHOOD, 500);
}

function showHOOD() {
    _watchlistAnimationRoot.addClass('detail-hood');
    setTimeout(tapHamburger, 2500);
}

function tapHamburger() {
    _watchlistPress.removeClass('long-tap');
    _watchlistPress.addClass('tap');
    _watchlistAnimationRoot.addClass('pre-to-portfolio');
    setTimeout(showPortfolio, 750);
}

function showPortfolio() {
    _watchlistPress.removeClass('tap');
    _watchlistAnimationRoot.removeClass('detail-view');
    _watchlistAnimationRoot.addClass('to-portfolio');
    _watchlistAnimationRoot.removeClass('pre-to-portfolio');
    setTimeout(holdOpenBuy, 2000);
}

function holdOpenBuy() {
    _watchlistPress.addClass('med-tap');
    setTimeout(openBuy, 500);
}

function openBuy() {
    _watchlistAnimationRoot.removeClass('detail-hood');
    _watchlistSlideBuy.show();
    _watchlistAnimationRoot.addClass('buy');
    setTimeout(removeTap, 1500);
}

function removeTap() {
    _watchlistPress.removeClass('med-tap');
    setTimeout(holdCloseBuy, 500);
}

function holdCloseBuy() {
    _watchlistPress.addClass('med-tap');
    setTimeout(closeBuy, 500);
}

function closeBuy() {
    _watchlistAnimationRoot.removeClass('buy');
    setTimeout(addMask, 1500);
}

function addMask() {
    if (_isDaytime) {
        $('#black_phonemask').addClass('fade-in');
    } else {
        $('#white_phonemask').addClass('fade-in');
    }
    setTimeout(flipSolarity, 500);
}

function flipSolarity() {
    if (_isDaytime) {
        _watchlistRoot.removeClass('white-bg');
        _watchlistRoot.addClass('nighttime');
    } else {
        _watchlistRoot.addClass('white-bg');
        _watchlistRoot.removeClass('nighttime');
    }
    _isDaytime = !_isDaytime;
    setTimeout(tapROBN, 500);
}
var _tradescreenRoot;
var _tradescreenPress;
var _tradescreenSolidCircle;
var _tradescreenLoadingCircle;
var _webkitFilterSupported = false;

function isWebkitFilterSupported() {
    var elem = document.body || document.element;
    return typeof elem.style['WebkitFilter'] == 'string';
}

function initTradescreen() {
    _webkitFilterSupported = isWebkitFilterSupported();
    _tradescreenRoot = $('#tradescreen_wrapper');
    _tradescreenRoot.show();
    _tradescreenPress = $('#tradescreen_press');
    _tradescreenSolidCircle = $('#tradescreen_solid_circle');
    _tradescreenLoadingCircle = $('#tradescreen_loading_circle');
    setTimeout(tapSix, 1000);
}

function tapSix() {
    tap();
    _tradescreenRoot.addClass('entered');
    setTimeout(tapPreview, 2000);
}

function tapPreview() {
    _tradescreenRoot.addClass('preview');
    tap();
    setTimeout(showNavBack, 400);
}

function showNavBack() {
    _tradescreenRoot.addClass('preview-nav');
    setTimeout(tapSubmit, 2000);
}

function tapSubmit() {
    _tradescreenRoot.addClass('submit');
    if (!_webkitFilterSupported) {
        _tradescreenRoot.addClass('noblur-submit');
    }
    tap();
    setTimeout(switchCircles, 2000);
}

function switchCircles() {
    _tradescreenSolidCircle.addClass('pop');
    _tradescreenLoadingCircle.addClass('spin');
    setTimeout(receiveOrder, 2000);
}

function receiveOrder() {
    _tradescreenRoot.addClass('received');
    setTimeout(confirmOrder, 2000);
}

function confirmOrder() {
    _tradescreenRoot.addClass('confirmed');
    _tradescreenLoadingCircle.addClass('pop');
    setTimeout(tradescreenReset, 4000);
}

function tradescreenReset() {
    _tradescreenRoot.removeClass('entered preview preview-nav submit noblur-submit received confirmed');
    _tradescreenSolidCircle.removeClass('pop');
    _tradescreenLoadingCircle.removeClass('spin pop');
    setTimeout(tapSix, 1500);
}

function tap() {
    _tradescreenPress.addClass('tap');
    setTimeout(function() {
        _tradescreenPress.removeClass('tap');
    }, 500);
}